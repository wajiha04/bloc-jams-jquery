class Player {
  ...
  // The skipTo() method has one parameter, percent. It checks if this.playState is different from playing.
  // If it is different, then it will return the sound object set time.

  skipTo (percent) {
    if (this.playState !== 'playing') { return }
    this.soundObject.setTime( (percent / 100) * this.soundObject.getDuration() );
  }
  // The setVolume has one parameter, percent and it sets the volume property to percent.
  setVolume (percent) {
    this.volume = percent;
    this.soundObject.setVolume(percent);
  }
}
// The const (constant) player is set to new Player
